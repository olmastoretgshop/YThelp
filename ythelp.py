This file is in google_auth_oauthlib library. The file is called Flow.py
...
import argparse, httplib2, os, random, time, subprocess, json
...
My old code is returning the url, but with error. This is the url it returned (let’s say it is my url):
https://accounts.google.com/o/oauth2/auth?response_type=code&client_id=96579846022-a2l6osi3kbe89ckr5abab4cvj4jagokd.apps.googleusercontent.com&scope=https%3A%2F%2Fwww.googleapis.com%2Fauth%2Fyoutube.upload&state=UcDUzC0NsYMjlAHq9EwDicl6kl1TTf&access_type=offline

This is what it is originally returning (let’s say it is google’s url):
https://accounts.google.com/o/oauth2/auth?response_type=code&client_id=96579846022-a2l6osi3kbe89ckr5abab4cvj4jagokd.apps.googleusercontent.com&redirect_uri=urn%3Aietf%3Awg%3Aoauth%3A2.0%3Aoob&scope=https%3A%2F%2Fwww.googleapis.com%2Fauth%2Fyoutube.upload&state=Iz5RCsnfoAKuym01hCyibtephbAWED&prompt=consent&access_type=offline

Green part of my url is the same as google’s url. Orange part of the url is random url. I am missing red part of url.


I want to rewrite my old code. In my new code I am trying to get proper auth url, same as google’s.

The way I understood the run_console function. This code
self.redirect_uri = self._OOB_REDIRECT_URI
in run_console function is calling redirect_uri function and assigning _OOB_REDIRECT_URI as value. 
@redirect_uri.setter
    def redirect_uri(self, value):
        if value in _OOB_REDIRECT_URIS:
            warnings.warn(
        ...
        self.oauth2session.redirect_uri = value
, which is checking if value, which is _OOB_REDIRECT_URI, which is
_OOB_REDIRECT_URI = "urn:ietf:wg:oauth:2.0:oob"
, is in _OOB_REDIRECT_URIS, then assigning and passing to another redirect_uri function
def redirect_uri(self):
        """The OAuth 2.0 redirect URI. Pass-through to
        ``self.oauth2session.redirect_uri``."""
        return self.oauth2session.redirect_uri
, which is join the value with oauth2session variable
self.oauth2session = oauth2session
, which is getting oauth2session from __init__ function’s argument
def __init__(
        self,
        oauth2session,
        client_type,
...
Then in run_console function, auth_url is generated joining final redurect_uri plus generated url, from authorization_url function, from Flow class
auth_url, _ = self.authorization_url(**kwargs)
, plus kwargs, which is
kwargs.setdefault("prompt", "consent")


All of this is resulting in full authorization url:
https://accounts.google.com/o/oauth2/auth?response_type=code&client_id=96579846022-a2l6osi3kbe89ckr5abab4cvj4jagokd.apps.googleusercontent.com&redirect_uri=urn%3Aietf%3Awg%3Aoauth%3A2.0%3Aoob&scope=https%3A%2F%2Fwww.googleapis.com%2Fauth%2Fyoutube.upload&state=Iz5RCsnfoAKuym01hCyibtephbAWED&prompt=consent&access_type=offline
...
def get_authenticated_service(**kwargs):
    ...
    url = redirect_uri.authorization_url(**kwargs)
    # return url
    # credentials = flow.run_console()
    # # url = credentials()
    print(url)
...
I am trying to do the same logic in my new code. I am passing _OOB_REDIRECT_URI to redirect_uri function and retrieving into redirect_uri variable
redirect_uri = _OOB_REDIRECT_URI
redirect_uri = flow.redirect_uri()
Then i am joining final redurect_uri and kwargs with url from authorization_url function
kwargs.setdefault("prompt", "consent")
url = redirect_uri.authorization_url(**kwargs)
Then i am printing the url
print(url)


But the code is giving me error message:
Traceback (most recent call last):
  File "G:\Acer Windows 10 Present - august\Projects\youtube manager\test 2\run_cmd.py", line 70, in <module>
	youtube = get_authenticated_service()
  File "G:\Acer Windows 10 Present - august\Projects\youtube manager\test 2\run_cmd.py", line 61, in get_authenticated_service
	redirect_uri = flow.redirect_uri()
TypeError: 'NoneType' object is not callable

Please help me solve the error of my code🙏
